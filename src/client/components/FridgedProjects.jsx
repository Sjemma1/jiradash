...
class FridgedProjects extends React.Component {
	constructor() {
		super();
	...
	isActive(value) {
  	...
  	render() {
			...
			var category = 'Fridged';

			projects.filter(function(el){
				return (el.category === category);
			}).map(function(el, index) {
				 projectarray.push(el);
			})

			return (
			<div>
				<div id="main">
					<h1>{category} Projects : {projectarray.length}</h1>
					{(projectarray.length === 0) ? <h1 className="empty-title">No {category} Projects</h1> : ''}
						<ul id="cards">
...
export default FridgedProjects;
